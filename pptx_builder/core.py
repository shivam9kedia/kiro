...
def create_presentation_rels(num_slides):
...
'''
    for i in range(num_slides):
        rels += f'  <Relationship Id="rId{i+10}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slides/slide{i+1}.xml"/>\n'
    rels += '</Relationships>'
    return rels


def create_presentation(num_slides):
    slide_list = ""
    for i in range(num_slides):
        slide_list += f'    <p:sldId id="{256+i}" r:id="rId{i+10}"/>\n'
    return f'''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
def create_theme():
    return '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="Apollo Theme">
  <a:themeElements>
    <a:clrScheme name="Apollo">
